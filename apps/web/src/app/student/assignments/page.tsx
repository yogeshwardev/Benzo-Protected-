...
type Assignment = { id: string; title: string; description: string; dueAt?: string | null; required: boolean; submissions: Submission[]; courseTitle: string };
...
export default function StudentAssignmentsPage() {
  ...
  async function load() { if (!enrollments.data) return; setLoading(true); setError(null); try { const groups = await Promise.all(enrollments.data.map(async (enrollment) => (await apiRequest<Omit<Assignment,"courseTitle">[]>(`/assignments/course/${enrollment.course.id}`)).map((item) => ({ ...item, courseTitle: enrollment.course.title })))); setItems(groups.flat()); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to load assignments."); } finally { setLoading(false); } }
